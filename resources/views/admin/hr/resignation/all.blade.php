...
<div class="kt-portlet kt-portlet--mobile">
	<div class="kt-portlet__head kt-portlet__head--lg">
      ...
      <span class="kt-portlet__head-icon"><i class="fab fa-gg-circle"></i></span>
			<h3 class="kt-portlet__head-title">All Resigantion</h3>
		</div>
		<div class="kt-portlet__head-toolbar">
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
                    @can('insert_resignation')
					<a href="{{route('add_resignation')}}" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-plus-circle"></i>Add Resignation</a>
                    @endcan
				</div>
			</div>
		</div>
		...
		<table class="table table-striped- table-bordered table-hover table-checkable custom_table" id="alltableinfo">
			...
			<tbody>
                @php
                    $i =1;
                @endphp
				@forelse ($resignation as $item)
				<tr>
					<td>{{ $i++ }}</td>
                    <td>{{ $item->employee->name }}</td>
					<td>{{ $item->employee->branch_details->name }}</td>
					<td>{{ $item->resignation_for }}</td>
					<td>{{ $item->notice_date }}</td>
					<td>{{ $item->resignation_date }}</td>

					<td>
                        @can('edit_resignation')
						<a href="{{route('edit_resignation', $item->id)}}" class="text-info" title="edit"><i class="fa fa-pen-square fa-2x"></i></a>
                        @endcan
