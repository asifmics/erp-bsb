...
<div class="kt-portlet kt-portlet--mobile">
	<div class="kt-portlet__head kt-portlet__head--lg">
      ...
      <span class="kt-portlet__head-icon"><i class="fab fa-gg-circle"></i></span>
			<h3 class="kt-portlet__head-title">All Publication</h3>
		</div>
		<div class="kt-portlet__head-toolbar">
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
                    @can('insert_publication')
					<a href="{{route('add_publication')}}" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-plus-circle"></i>Add Publication</a>
                    @endcan
				</div>
			</div>
		</div>
		...
		<table class="table table-striped- table-bordered table-hover table-checkable custom_table" id="alltableinfo">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $item->remark}}</td>
					<td>
                        @can('view_publication')
						<a href="{{route('view_publication', $item->id)}}" class="text-info" title="edit"><i class="fa fa-eye fa-2x"></i></a>
                        @endcan
                        @can('edit_publication')
						<a href="{{route('edit_publication', $item->id)}}" class="text-info" title="edit"><i class="fa fa-pen-square fa-2x"></i></a>
                        @endcan
                        @can('delete_publication')
				...
				@empty
					<tr>
						<td colspan="7" class="text-center">No Publication found</td>
					</tr>
				@endforelse
...
<div class="modal fade" id="softDelModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
				<form method="post" action="{{route('softdelete_publication')}}">
					@csrf
